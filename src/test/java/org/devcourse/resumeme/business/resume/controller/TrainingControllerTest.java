...
class TrainingControllerTest extends ControllerUnitTest {
    ...
    @Test
    void 트레이닝_수정에_성공한다() throws Exception {
        // given
        TrainingCreateRequest request = new TrainingCreateRequest(
                "데브대", "컴퓨터공학과", "학사 학위", LocalDate.of(2018, 3, 1),
                LocalDate.of(2022, 2, 28), 4.0, 4.5, "성적 우수"
        ...
        // when
        ResultActions result = mvc.perform(patch("/api/v1/resume/" + resumeId + "/trainings/components/{componentId}", componentId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(toJson(request))
        );
        ...
        result
                .andExpect(status().isOk())
                .andDo(
                        document("resume/training/update",
                                getDocumentRequest(),
                                getDocumentResponse(),
                                requestFields(
                                        fieldWithPath("type").type(STRING).description("서버쪽에서 동적으로 처리합니다 보내지 마세요").optional(),
                                ...
                                responseFields(
                                        fieldWithPath("id").type(NUMBER).description("수정된 트레이닝 ID")
                                )
                        )
                );
    }
